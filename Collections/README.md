...
| `SPRING_BOOT_GROOVY_ERUDIO.postman_environment.json` | Variáveis usadas pela collection |
| `files/` | Arquivos de exemplo usados nos uploads (`sample.txt`, `people.csv`, `people.xlsx`) |

## Usando no Postman

1. Importe a collection e o environment e selecione o environment `SPRING_BOOT_GROOVY_ERUDIO`.
...
npx newman run Collections/*.postman_collection.json \
  -e Collections/SPRING_BOOT_GROOVY_ERUDIO.postman_environment.json \
  --working-dir Collections
```

Use `--env-var baseUrl=http://localhost:8085` para outra porta e `--folder person` para uma pasta só.

O mesmo comando roda no GitHub Actions (`.github/workflows/continuous-deployment.yml`): depois do `./gradlew clean build`, o workflow sobe o `docker compose`, espera a API responder e executa o newman antes de publicar a imagem. Se um teste da collection falhar, a imagem não é publicada.
